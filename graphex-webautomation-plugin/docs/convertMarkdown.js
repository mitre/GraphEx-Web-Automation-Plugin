...
const INPUT_FILEPATH = path.resolve(
  __dirname,
  DOCS_RELATIVE_PATH + INPUT_DIR_NAME
)
const OUTPUT_DIR_NAME = 'html'
const OUTPUT_LOCATION = path.resolve(
  __dirname,
  DOCS_RELATIVE_PATH + OUTPUT_DIR_NAME
)
...
function updateImgPaths(htmlString, filePath) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    while (indexLoc > -1) {
      ...
      const currentSrc = imgTag.slice(
        srcStartIndex + 5,
        imgTag.indexOf('"', srcStartIndex + 6)
      )
      const newSrc =
        filePath.slice(0, filePath.lastIndexOf('/') + 1) + currentSrc
...
// convert the files
console.log(
  'Converting markdown documentation files from path:',
  INPUT_FILEPATH
)
  ...
  if (fs.statSync(filepath).isFile()) {
    if (filepath.endsWith('.md')) {
      let html = marked.parse(
        fs.readFileSync(filepath).toString().replaceAll('.md', '.html'),
        {
